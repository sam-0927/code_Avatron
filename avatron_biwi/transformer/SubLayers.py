...
class ScaledDotProductAttention1(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):  # [B,L,256]
        attn = torch.bmm(q, k.transpose(1, 2))
        attn = attn / self.temperature
...
class ScaledDotProductAttention2(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, k_mask=None):  # [B,L,256]
        attn = torch.bmm(q, k.transpose(1, 2))
        attn = attn / self.temperature  # [B, T, T]
        if mask is not None:
            attn = attn.masked_fill(mask, -np.inf)
        attn = self.softmax(attn)
        '''
		if k_mask is not None:
            v = v.masked_fill(k_mask, 0)
		'''
        output = torch.bmm(attn, v)
        output = output.masked_fill(k_mask, 0)
        return output, attn
...
class MultiHeadAttention2(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, k_mask=None):
        d_k, d_v, n_head = self.d_k, self.d_v, self.n_head
        ...
        k_mask = k_mask.repeat(n_head, 1, 1) if k_mask is not None else None  # (n*b) x .. x ..

        output, attn = self.attention(q, k, v, mask=mask, k_mask=k_mask)
...
class MultiHeadAttention1(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.attention = ScaledDotProductAttention1(
            temperature=np.power(d_k, 0.5))
        self.layer_norm = nn.LayerNorm(d_model)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, dropout=0.1):
        ...
        # Use Conv1D
        # position-wise
        self.depthwisecnn = nn.Conv1d(d_in, d_hid, kernel_size=hp.fft_conv1d_kernel_size[0], padding=(hp.fft_conv1d_kernel_size[0]-1)//2)
        # position-wise
        self.w_2 = nn.Conv1d(
    ...
    def forward(self, x):
        ...
        output = self.w_2(F.relu(self.depthwisecnn(output)))
