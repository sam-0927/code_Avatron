...
def trainer(args, train_loader, dev_loader, model, tts_model, optimizer, steplr, criterion, epoch=100):
    ...
    for e in range(init_e, epoch+1):
        ...
        optimizer.zero_grad()
        times = []
        for i, (mel, vertice, template, one_hot, text, pho_label, f0, energy, file_name, speaker) in pbar:
            iteration += 1
            # to gpu
            ...
            loss, pred_vert = \
            model(up_emb.detach(), pho_label, hard_A, template,  vertice, one_hot, criterion, args.up_pho)
        ...
        with torch.no_grad():
            for mel, vertice, template, one_hot_all, text, pho_label, f0, energy, file_name, speaker in dev_loader:
                ...
                # v_plot_attn(writer, pros_attn, soft_A, hard_A, iteration, 0)

                # Add mel-spectrogram and attention map and alignment
                ...
                if condition_subject in train_subjects_list:
                    ...
                    for iter in range(one_hot_all.shape[-1]):
                        ...
                        one_hot = one_hot_all[:,iter,:]
                        loss, v_pred_vert = \
                        model(up_emb, template, vertice, one_hot, criterion)
                end = time.time()
            ...
            writer.add_loss(np.mean(valid_loss_log), iteration, 'Val/vertices')
