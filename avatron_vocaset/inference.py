...
def synthesizer(args, dev_loader, model, tts_model):
    ...
    with torch.no_grad():
        for mel, vertice, template, one_hot_all, text, _, pho_label, f0, energy, file_name, speaker in dev_loader:
            ...
            ###### TTS forward
            mel_output, d_prediction, up_emb = tts_model.inference(None, text, src_len, None, None, style_emb)
            ...
            # Synthesize speech
            if len(mel_output.size()) != 3:
                mel_output = mel_output.unsqueeze(0)
            mel_output = mel_output.transpose(1,2)

            # reference
            y_g_hat = vocoder(mel.transpose(1,2))            
            ...
            if condition_subject in train_subjects_list:
                ...
                pred_vert = model.predict(up_emb, up_pho_emb, template, one_hot)
                np.save(os.path.join(result_path,file_name[0].split('.')[0]+'.npy'), pred_vert.squeeze(0).detach().cpu().numpy())
            else:
                for iter in range(one_hot_all.shape[-1]):
                    condition_subject = train_subjects_list[iter]
                    one_hot = one_hot_all[:,iter,:]
                    pred_vert = model.predict(up_emb, up_pho_emb, template, one_hot, audio.shape[0], args.up_pho)

                    np.save(os.path.join(result_path,file_name[0].split('.')[0]+'_cond_'+condition_subject.split('_')[2]+'.npy'), pred_vert.squeeze(0).detach().cpu().numpy())
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="save/save_try2", help='path of the trained models')
