...
from text import *
from text import _arpabet_to_sequence, _curly_re
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # seq_len, fea_dim
        file_name = self.data[index]["name"]    # *_sentence00_enhanced
        speaker = self.data[index]["spk"]
        audio = self.data[index]["audio"]
        text = self.data[index]["text"]
        raw_text = self.data[index]["raw_text"]
        ...
        torch.LongTensor(text), raw_text, torch.LongTensor(pho_label), torch.FloatTensor(f0), torch.FloatTensor(energy), file_name, speaker
...
def read_data(args, hp):
    ...
    with open(filepath, 'r') as fd:
        ...
        for f in tqdm(fs):
            ...
            data_base_path_list = wav_path.split('/')
            data_base_path = '/'.join(data_base_path_list[:-2])
            ...
            raw_text = _curly_re.match(text).group(2).split()
            ...
            text_values = text
            f = "_".join(name.split("_")[:-1]) + '.wav'
            key = f.replace('wav', 'npy')
            data[key]["raw_text"] = raw_text
